  ...
  },
  {
    ...
    preview: (
      <div className="grid grid-cols-3 gap-2">
        {[
          ["Clear", "85%", "text-emerald-300 bg-emerald-500/10"],
          ["Review", "12%", "text-sky-300 bg-sky-500/10"],
          ["Stop", "3%", "text-red-300 bg-red-500/10"],
        ].map(([label, pct, tone]) => (
  ...
  },
  {
    ...
    preview: (
      <div className="flex items-center gap-3">
        ...
        <div className="grid size-12 place-items-center rounded-full border border-indigo-500/20 bg-indigo-500/10 text-[10px] font-semibold text-indigo-300">
          FIC
        </div>
      </div>
    ),
...
export function Features() {
  return (
    <section
      className="scroll-mt-24 border-y border-dashed border-white/[0.06] bg-[#03040a]/70 py-24 text-white md:py-32"
      id="security"
    >
      <div className="container mx-auto">
        <section className="flex flex-col gap-4 lg:flex-row lg:items-baseline lg:gap-x-20">
          <h2 className="font-display max-w-[700px] text-4xl font-medium leading-tight text-white text-pretty md:text-5xl">
      ...
      <div className="mt-20 border-y border-dashed border-white/[0.06]">
        <div className="container grid grid-cols-1 overflow-hidden sm:grid-cols-2 lg:grid-cols-4">
          {features.map((box) => (
            ...
            >
              <div className="mb-5 h-28 overflow-hidden rounded-xl border border-white/[0.07] bg-[radial-gradient(circle_at_20%_0%,rgba(101,115,255,0.16),transparent_34%),rgba(255,255,255,0.035)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
                {box.preview}
              </div>
