...
const checkedDirs = ["dashboard", "matters", "settings", "internal"].map((dir) => join(appDir, dir));
...
describe("product UI quality", () => {
  const files = checkedDirs.flatMap(collectTsxFiles);

  it("keeps customer pages independent from internal analyst components", () => {
    const customerFiles = files.filter((file) => file.includes("/matters/") || file.includes("/settings/"));
  ...
  it("keeps page-level product routes on shared icon components", () => {
    ...
    const offenders = pageFiles.filter((file) => readFileSync(file, "utf-8").includes("<svg"));
  ...
  it("keeps large rounded product cards out of route pages", () => {
    const pageFiles = files.filter((file) => file.endsWith("/page.tsx"));
    const offenders = pageFiles.filter((file) => readFileSync(file, "utf-8").includes("rounded-2xl"));
